...
<head>
    ...
    <link rel="icon" href="{{ secure_asset('assets/LOGO.png') }}" type="image/png" />
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            searchForm.addEventListener('submit', function(event) {
                ...
                if (query.length > 0) {
                    fetch(`https://e922-2404-c0-2420-00-759d-f4f0.ngrok-free.app/user/form-compare?search=${query}`, {
                            method: 'GET',
                            headers: {
                        ...
                        .then(data => {
                            if (data.films.length > 0) {
                                ...
                                <button data-id="${film.id}" data-title="${film.judul}"
                                    data-poster="{{ secure_asset('storage/${film.poster}') }}"
                                    class="film-result flex items-center gap-4 p-2 border border-gray-300 rounded hover:bg-gray-100 w-full text-left">
                                    <img src="{{ secure_asset('storage/${film.poster}') }}" alt="${film.judul}"
                            ...
                            `).join('');
                                ...
                                searchResults.innerHTML =
                                    '<p class="text-gray-500">Film yang Anda cari tidak ada atau belum tersedia.</p>';
                            }
                        });
                } else {
                    searchResults.innerHTML = '';
